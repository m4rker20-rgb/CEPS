...
## Где находится anti-AI guard

Блок `CEPS AI-GUARD` находится в корневом файле [`ceps_installer.py`](ceps_installer.py). Он содержит маркированные notice-строки и SHA-256 seal: при изменении этих строк loader останавливается до расшифровки payload. Во время сборки guard также попадает в runtime-шаблон, поэтому его можно увидеть в итоговом loader-файле.

Это дополнительная проверка целостности против автоматического редактирования и не самостоятельная «защита от ИИ»: модель или человек всё равно могут прочитать файл. Реальную защиту релиза обеспечивают Ed25519, HMAC, Merkle/hash-chain, шифрование и CEPS-3 env binding.
