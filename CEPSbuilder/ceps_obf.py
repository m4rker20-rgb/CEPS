...
# "Невидимые" идентификаторы: U+3164 (HANGUL FILLER) — категория Unicode Lo,
# т.е. для компилятора Python это настоящая буква, но выглядит как пробел.
# U+115F (HANGUL CHOSEONG FILLER) — второй символ для различения имён.
# Включается конфигом obfuscation: invisibleNames: true (по умолчанию false —
# при отладке стектрейсы становятся нечитаемыми).
_INVISIBLE_STARTERS = "\u3164\u115F"
_INVISIBLE_REST = "\u3164\u115F\u1160"


def _makeObfName(usedNames: set[str], invisible: bool = False) -> str:
    # gen a random ident
    if invisible:
        counter = len(usedNames)
        while True:
            n = counter
            tail = ""
            while True:
                tail = _INVISIBLE_REST[n % 3] + tail
                n = n // 3
                if n == 0:
                    break
            name = _INVISIBLE_STARTERS[counter % 2] + tail
            counter += 1
...
class RenameLocals(ast.NodeTransformer):
    def __init__(self, protectedNames: frozenset[str], localClassNames: frozenset[str] = frozenset(),
                 invisibleNames: bool = False) -> None:
        self.protectedNames = protectedNames
        self.localClassNames = localClassNames
        self._invisibleNames = invisibleNames
    ...
    def _renameFunction(self, node: ast.AST) -> ast.AST:
        ...
        def nextName() -> str:
            return _makeObfName(usedNames, getattr(self, "_invisibleNames", False))
...
def applyRenameLocals(source: str, protectedNames: frozenset[str], invisibleNames: bool = False) -> str:
    tree = ast.parse(source)
    tree = RenameLocals(protectedNames, invisibleNames=invisibleNames).visit(tree)
    ast.fix_missing_locations(tree)
    return ast.unparse(tree)
...
def _string_table_tree(tree) -> str:
    """AST-проход: длинные строковые литералы -> _ST(i); возвращает преамбулу
    с закодированной таблицей. Дерево модифицируется in place."""
    import ast as _ast
    import base64 as _b64
    import zlib as _zlib

    owners = set()  # id docstring-узлов (первый stmt модуля/класса/функции)
    for n in _ast.walk(tree):
        if isinstance(n, (_ast.FunctionDef, _ast.AsyncFunctionDef, _ast.ClassDef, _ast.Module)):
            if n.body and isinstance(n.body[0], _ast.Constant) and isinstance(n.body[0].value, str):
                owners.add(id(n.body[0]))

    order: list[str] = []

    class Replace(_ast.NodeTransformer):
        def visit_Constant(self, node):
            if (isinstance(node.value, str)
                    and len(node.value) >= _MIN_ST_LENGTH
                    and id(node) not in owners):
                order.append(node.value)
                idx = len(order) - 1
                return _ast.copy_location(_ast.parse(f"_ST({idx})").body[0].value, node)
            return node
    ...
    if not order:
        return ""
    blob = _zlib.compress("\x00".join(order).encode("utf-8"), 9)
    b64 = _b64.b64encode(blob).decode("ascii")
    lines = [b64[i:i + 96] for i in range(0, len(b64), 96)]
    return (
        "import base64 as _st_b, zlib as _st_z\n"
        "_ST_RAW = (\n" +
        "".join('    "%s"\n' % ln for ln in lines) +
        ")\n"
        "_ST_CACHE = tuple(_st_z.decompress(_st_b.b64decode(_ST_RAW)).decode('utf-8').split('\\x00'))\n"
        "def _ST(i):\n"
        "    return _ST_CACHE[i]\n"
    )


def applyStringTable(source: str) -> str:
    """Выносит длинные строковые литералы (base64-payload и т.п.) в одну
    zlib-сжатую таблицу _S. В коде остаются вызовы _S(0), _S(1)...

    Зачем: XOR-энкодинг гигантского base64 раздувает файл в ~15 раз и
    компилятор на телефоне захлёбывается. Таблица сжимает повторяющиеся
    блоки base64 в разы и парсится мгновенно (один литерал).

    Безопасность: не трогает docstring-позиции (первый stmt), f-строки уже
    восстановлены, байтовые литералы пропускает.
    """
    import ast as _ast
    import base64 as _b64
    import zlib as _zlib
    ...
    owners = {}  # id(docstring-узла) — первый stmt модуля/класса/функции
    for n in _ast.walk(tree):
        if isinstance(n, (_ast.FunctionDef, _ast.AsyncFunctionDef, _ast.ClassDef, _ast.Module)):
            if n.body and isinstance(n.body[0], _ast.Constant) and isinstance(n.body[0].value, str):
                owners[id(n.body[0])] = True
    ...
    out = table_src + _ast.unparse(tree)
    compile(out, "<stringtable>", "exec")  # самопроверка
    return out
...
def applyObfuscationPipeline(source: str, protectedNames: frozenset[str], xorKey: int, localClassNames: frozenset[str] = frozenset(), obfConfig: dict | None = None) -> str:
    ...
    doInvisibleNames: bool = obfConfig.get("invisibleNames", False)
    doStringTable: bool = obfConfig.get("stringTable", True)
    ...
    if doRemoveLogs:
        tree = RemoveLogs(commentLines["# ELYBsaveLog"]).visit(tree)
    if doRenameLocals:
        tree = RenameLocals(allProtected, localClassNames, invisibleNames=doInvisibleNames).visit(tree)
    table_preamble = ""
    if doStringTable:
        # ДО encodeStrings: длинные литералы (base64-payload и т.п.) уходят в
        # сжатую таблицу _ST, XOR остаётся только для коротких строк. Иначе
        # XOR-энкодинг гигантских base64 раздувает файл в ~15 раз.
        table_preamble = _string_table_tree(tree)
    if doEncodeStrings:
